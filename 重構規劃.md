# 🔧 金流串接系統 - 完整重構規劃指南

> 📅 建立日期：2025-12-01
> 👥 目標對象：Junior 開發者
> 🎯 目標：整理程式碼、提升可維護性、完整中文化

請絕對不要使用中文變數。
---

## 📊 專案現況分析

### ✅ 優點
- 清晰的模組化結構（routes、services、middleware、utils）
- 良好的安全性配置（CSRF、Rate Limit、Helmet）
- 完整的訂閱制和一次性付款支援
- Google Sheets 作為資料庫的創新解決方案
- 功能測試都正常運作

### ⚠️ 需改進項目
- 前端 JavaScript 缺乏模組化（app.js 400+ 行）
- 部分業務邏輯重複
- 文檔過多且分散
- 缺少 JSDoc 註釋
- 錯誤處理不一致
- **英文與中文混用，不夠友好**

---

## 🎯 重構目標

1. **文檔精簡**：只保留 1-2 份關鍵索引文件
2. **程式碼模組化**：拆分前端 JavaScript，提升可維護性
3. **中文化**：全面繁體中文友好化
4. **程式碼品質**：統一命名、新增註釋、強化錯誤處理

---

# 第一部分：原始重構規劃

## 📝 階段一：文檔整理（優先級：HIGH）

### Task 1.1：建立單一索引文件

**目標**：將所有文檔整合成一份 `README.md`

**步驟**：

1. 在專案根目錄建立新的 `README.md`

2. 包含以下章節：
```markdown
# 金流串接系統

## 快速開始
- 環境變數設定
- 安裝依賴
- 初始化 Google Sheets
- 啟動服務

## 架構說明
- 資料夾結構
- 技術棧
- 資料流向

## API 文檔
- 認證 API
- 支付 API
- 訂閱 API
- Webhook

## 開發指南
- 新增商品
- 測試支付流程
- 訂閱制配置

## 部署
- 環境變數清單
- Google Cloud 設定
- PAYUNi 設定
```

3. **刪除以下冗餘文檔**（內容已整合）：
   - ❌ `code-review-agent.md`
   - ❌ `CodeMender.md`
   - ❌ `improve.md`
   - ❌ `planning-agent.md`
   - ❌ `google-console.md`（內容移至 README）
   - ❌ `Notion_Page_Structure.md`（非程式碼相關）

### Task 1.2：保留關鍵配置文檔

**保留**：
- ✅ `.env.example`（環境變數範本）
- ✅ `README.md`（新建的索引文檔）
- ✅ `重構規劃.md`（本文件）

---

## 🔧 階段二：後端程式碼重構（優先級：MEDIUM）

### Task 2.1：提取常數與配置

**文件**：`src/config/constants.js`

**改進點**：
```javascript
// 現狀：constants.js 已經做得很好
// 建議：新增 JSDoc 註釋

/**
 * 伺服器配置
 * @typedef {Object} ServerConfig
 * @property {number} PORT - 伺服器監聽埠號
 * @property {string} NODE_ENV - 執行環境
 * @property {string} DOMAIN - 網域名稱
 */
export const SERVER_CONFIG = {
  PORT: 80,
  NODE_ENV: process.env.NODE_ENV || "development",
  DOMAIN: process.env.DOMAIN || "https://example.com",
};
```

### Task 2.2：優化 PayuniSDK

**文件**：`src/services/payment/PayuniSDK.js`

**改進點**：

1. **提取共用邏輯**
```javascript
// 現狀：重複的錯誤處理
// 建議：提取為私有方法
_handleApiError(error, context) {
  logger.error(`${context} 失敗`, { error: error.message });
  return {
    success: false,
    error: error.message,
  };
}
```

2. **統一回應格式**
```javascript
// 建議：所有 async 方法都返回標準格式
{
  success: boolean,
  data?: any,
  error?: string
}
```

### Task 2.3：簡化 Webhook 處理邏輯

**文件**：`src/routes/payment.js:131`

**改進點**：
```javascript
// 現狀：payuni-webhook 處理邏輯過長（150+ 行）
// 建議：提取為獨立 service

// 新建：src/services/webhookHandler.js
export class WebhookHandler {
  async processPayment(webhookData) {
    // 1. 驗證資料
    // 2. 查詢訂單
    // 3. 更新訂單
    // 4. 授予權益
  }
}
```

### Task 2.4：強化錯誤處理

**文件**：`src/utils/errors.js`

**改進點**：
```javascript
// 新增更多語義化的錯誤類型
export const PaymentErrors = {
  // 現有的...

  ProductNotFound: (productId) =>
    new PaymentError(404, '商品不存在', { productId }),

  OrderAlreadyPaid: (tradeNo) =>
    new PaymentError(409, '訂單已支付', { tradeNo }),

  InvalidWebhookSignature: () =>
    new PaymentError(401, 'Webhook 簽章驗證失敗'),
};
```

---

## 🎨 階段三：前端程式碼重構（優先級：HIGH）

### Task 3.1：模組化前端 JavaScript

**文件**：`_frontend/public/app.js`

**現狀問題**：
- 單一檔案 412 行
- 全域變數污染
- 難以維護和測試

**重構方案**：拆分為多個模組

#### 步驟 1：建立模組資料夾結構

```
_frontend/public/
├── js/
│   ├── modules/
│   │   ├── auth.js        # 認證邏輯
│   │   ├── products.js    # 商品邏輯
│   │   ├── payment.js     # 支付邏輯
│   │   ├── orders.js      # 訂單歷史
│   │   └── ui.js          # UI 工具函數
│   └── main.js            # 主進入點
├── app.js                 # (刪除，改為 main.js)
└── ...
```

#### 步驟 2：拆分 auth.js

```javascript
// _frontend/public/js/modules/auth.js
export class AuthManager {
  constructor() {
    this.currentUser = null;
  }

  async checkLoginStatus() {
    try {
      const res = await fetch("/api/me");
      if (!res.ok) return null;
      const data = await res.json();
      return data.loggedIn ? data.user : null;
    } catch (e) {
      return null;
    }
  }

  updateUI(user, elements) {
    this.currentUser = user;

    if (user) {
      elements.guestView.classList.add("hidden");
      elements.userView.classList.remove("hidden");
      elements.userAvatar.src = user.picture || "";
      elements.userName.textContent = user.name;

      // Check subscription
      const hasActiveSubscription = user.entitlements?.some(
        e => e.type === 'subscription' && e.status === 'active'
      );
      if (hasActiveSubscription) {
        this._addPremiumBadge(elements.userName);
      }
    } else {
      elements.userView.classList.add("hidden");
      elements.guestView.classList.remove("hidden");
    }
  }

  _addPremiumBadge(element) {
    const badge = document.createElement("span");
    badge.className = "status-badge status-active";
    badge.style.marginLeft = "8px";
    badge.style.fontSize = "12px";
    badge.textContent = "Premium";
    element.appendChild(badge);
  }
}
```

#### 步驟 3：拆分 products.js

```javascript
// _frontend/public/js/modules/products.js
export class ProductManager {
  constructor() {
    this.products = [];
    this.selectedProductId = null;
    this.currentTab = 'subscription';
  }

  async fetchProducts() {
    const res = await fetch("/api/products");
    if (!res.ok) throw new Error("Failed to load products");
    this.products = await res.json();
    return this.products;
  }

  setTab(tab) {
    this.currentTab = tab;
  }

  getFilteredProducts() {
    return this.products.filter(p => {
      if (this.currentTab === 'subscription') {
        return p.type === 'subscription' || !p.type;
      }
      return p.type === 'one_time';
    });
  }

  selectProduct(productId) {
    this.selectedProductId = productId;
    const product = this.products.find(p => p.id === productId);
    return product;
  }

  renderProducts(container, currentUser) {
    container.innerHTML = "";
    const filtered = this.getFilteredProducts();

    if (filtered.length === 0) {
      container.innerHTML = `<div class="error show">沒有找到商品</div>`;
      return;
    }

    filtered.forEach(product => {
      if (this.currentTab === 'subscription') {
        this._renderSubscriptionCard(product, container, currentUser);
      } else {
        this._renderOneTimeCard(product, container, currentUser);
      }
    });
  }

  _renderSubscriptionCard(product, container, currentUser) {
    // 實作訂閱卡片渲染...
  }

  _renderOneTimeCard(product, container, currentUser) {
    // 實作一次性購買卡片渲染...
  }
}
```

#### 步驟 4：拆分 payment.js

```javascript
// _frontend/public/js/modules/payment.js
export class PaymentManager {
  constructor(csrfToken, clientConfig) {
    this.csrfToken = csrfToken;
    this.clientConfig = clientConfig;
  }

  async createPayment(productId, products) {
    const product = products.find(p => p.id === productId);
    if (!product) throw new Error("商品不存在");

    const turnstileToken = turnstile.getResponse();

    if (this.clientConfig.turnstileEnable && !turnstileToken) {
      throw new Error("請完成安全驗證");
    }

    const endpoint = product.type === 'subscription'
      ? '/create-subscription'
      : '/create-payment';

    const res = await fetch(endpoint, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        "X-CSRF-Token": this.csrfToken,
      },
      body: JSON.stringify({
        productID: productId,
        turnstileToken: turnstileToken
      }),
    });

    const data = await res.json();
    if (!res.ok) throw new Error(data.error || "支付建立失敗");

    return data;
  }

  redirectToPayment(paymentData) {
    const { payUrl, data } = paymentData;
    const form = document.createElement("form");
    form.method = "POST";
    form.action = payUrl;

    Object.entries(data).forEach(([key, value]) => {
      const input = document.createElement("input");
      input.type = "hidden";
      input.name = key;
      input.value = value;
      form.appendChild(input);
    });

    document.body.appendChild(form);
    form.submit();
  }
}
```

#### 步驟 5：建立 ui.js 工具模組

```javascript
// _frontend/public/js/modules/ui.js
export function showLoading() {
  const modal = document.getElementById("loading-modal");
  modal?.classList.add("show");
}

export function hideLoading() {
  const modal = document.getElementById("loading-modal");
  modal?.classList.remove("show");
}

export function showError(message) {
  const errorEl = document.getElementById("error-message");
  if (errorEl) {
    errorEl.textContent = message;
    errorEl.classList.add("show");
  }
  hideLoading();
}

export function clearError() {
  const errorEl = document.getElementById("error-message");
  errorEl?.classList.remove("show");
}
```

#### 步驟 6：建立主進入點 main.js

```javascript
// _frontend/public/js/main.js
import { AuthManager } from './modules/auth.js';
import { ProductManager } from './modules/products.js';
import { PaymentManager } from './modules/payment.js';
import { showLoading, hideLoading, showError, clearError } from './modules/ui.js';

document.addEventListener("DOMContentLoaded", async () => {
  // 初始化管理器
  const authManager = new AuthManager();
  const productManager = new ProductManager();
  let paymentManager;

  // UI Elements
  const elements = {
    // ... 所有 DOM 元素
  };

  // 初始化
  showLoading();

  const [csrfRes, configRes] = await Promise.all([
    fetch("/csrf-token"),
    fetch("/api/client-config")
  ]);

  const csrfData = await csrfRes.json();
  const clientConfig = await configRes.json();

  paymentManager = new PaymentManager(csrfData.csrfToken, clientConfig);

  const user = await authManager.checkLoginStatus();
  await productManager.fetchProducts();

  authManager.updateUI(user, elements);
  productManager.renderProducts(elements.productOptions, user);

  hideLoading();

  // Event Listeners
  elements.payBtn.addEventListener("click", async () => {
    try {
      clearError();
      elements.payBtn.disabled = true;
      showLoading();

      const paymentData = await paymentManager.createPayment(
        productManager.selectedProductId,
        productManager.products
      );

      paymentManager.redirectToPayment(paymentData);
    } catch (error) {
      showError(error.message);
      elements.payBtn.disabled = false;
    }
  });
});
```

#### 步驟 7：更新 HTML 引用

```html
<!-- _frontend/index.html -->
<script type="module" src="/js/main.js"></script>
```

---

## 📈 階段四：程式碼品質提升（優先級：MEDIUM）

### Task 4.1：新增 JSDoc 註釋

**範例**：
```javascript
/**
 * 建立支付訂單
 * @param {Object} req - Express 請求物件
 * @param {Object} req.body - 請求主體
 * @param {string} req.body.productID - 商品 ID
 * @param {string} req.body.turnstileToken - Turnstile 驗證 Token
 * @param {Object} res - Express 回應物件
 * @param {Function} next - 下一個中間件
 * @returns {Promise<void>}
 */
router.post("/create-payment", paymentLimiter, async (req, res, next) => {
  // ...
});
```

### Task 4.2：統一命名規範

**規則**：
- 檔案名稱：kebab-case（`payment-helpers.js`）
- 類別名稱：PascalCase（`PayuniSDK`）
- 函數名稱：camelCase（`createOrder`）
- 常數：UPPER_SNAKE_CASE（`SERVER_CONFIG`）

### Task 4.3：新增單元測試（選做）

**建議工具**：Vitest 或 Jest

**測試範例**：
```javascript
// tests/utils/paymentHelpers.test.js
import { describe, it, expect } from 'vitest';
import { generateTradeNo } from '../src/utils/paymentHelpers.js';

describe('paymentHelpers', () => {
  describe('generateTradeNo', () => {
    it('should generate unique trade numbers', () => {
      const tradeNo1 = generateTradeNo();
      const tradeNo2 = generateTradeNo();
      expect(tradeNo1).not.toBe(tradeNo2);
    });

    it('should match expected format', () => {
      const tradeNo = generateTradeNo();
      expect(tradeNo).toMatch(/^PAY\d{17}$/);
    });
  });
});
```

---

# 第二部分：繁體中文友好化改造

## 🌏 階段五：全面繁體中文化（優先級：HIGH）

### 中文化原則

- ✅ 所有註釋使用繁體中文
- ✅ 所有使用者可見訊息使用繁體中文
- ✅ 變數名稱可選：保持英文 + 中文註釋
- ✅ 錯誤訊息、日誌訊息全部繁體中文
- ⚠️ 技術術語保留英文（API、webhook、token、CSRF 等）

### Task 5.1：後端常數與配置中文化

**文件**：`src/config/constants.js`

**改造方案 A：保留英文 + 中文註釋**（推薦）

```javascript
/**
 * 伺服器配置
 */
export const SERVER_CONFIG = {
  /** 伺服器監聽埠號 */
  PORT: 80,
  /** 執行環境（開發/正式） */
  NODE_ENV: process.env.NODE_ENV || "development",
  /** 網域名稱 */
  DOMAIN: process.env.DOMAIN || "https://example.com",
};

/**
 * PAYUNi 金流配置
 */
export const PAYUNI_CONFIG = {
  /** API 網址 */
  API_URL: process.env.PAYUNI_API_URL,
  /** 商店 ID */
  MERCHANT_ID: process.env.PAYUNI_MERCHANT_ID,
  /** 雜湊密鑰 */
  HASH_KEY: process.env.PAYUNI_HASH_KEY,
  /** 雜湊向量 */
  HASH_IV: process.env.PAYUNI_HASH_IV,
  /** 返回網址 */
  RETURN_URL: process.env.PAYUNI_RETURN_URL || "https://example.com",
};
```
### Task 5.2：錯誤訊息中文化

**文件**：`src/utils/errors.js`

### Task 5.3：日誌訊息中文化

**重點**：所有 `logger` 呼叫都使用中文

**範例改造**：

```javascript
// ❌ 改造前
logger.info("Payment created successfully", { tradeNo, amount });
logger.error("Failed to create order", { error: error.message });

// ✅ 改造後
logger.info("支付訂單建立成功", { 訂單編號: tradeNo, 金額: amount });
logger.error("建立訂單失敗", { 錯誤訊息: error.message });
```

### Task 5.4：前端模組中文化

**文件**：`_frontend/public/js/modules/auth.js`

### Task 5.5：HTML 介面中文化

**文件**：`_frontend/index.html`

**改造重點**：所有使用者可見文字都使用繁體中文

```html
<!-- 步驟 1：登入驗證 -->
<section id="auth-section" class="step-section">
  <h2 class="step-title">
    <span class="step-number">1</span>
    登入驗證
  </h2>

  <!-- 訪客視圖 -->
  <div id="guest-view" class="auth-view">
    <p class="info-text">請先登入 Google 帳號以繼續購買流程</p>
    <button id="login-btn" class="btn btn-primary">
      使用 Google 帳號登入
    </button>
  </div>

  <!-- 已登入視圖 -->
  <div id="user-view" class="auth-view hidden">
    <div class="user-info">
      <img id="user-avatar" src="" alt="使用者頭像" class="user-avatar">
      <span id="user-name" class="user-name"></span>
    </div>
    <div class="user-actions">
      <button id="my-orders-btn" class="btn btn-secondary">我的訂單</button>
      <button id="logout-btn" class="btn btn-ghost">登出</button>
    </div>
  </div>
</section>

<!-- 步驟 2：選擇方案 -->
<section id="plan-section" class="step-section disabled">
  <h2 class="step-title">
    <span class="step-number">2</span>
    選擇方案
  </h2>

  <div class="tabs">
    <button class="tab-btn active" data-tab="subscription">訂閱制</button>
    <button class="tab-btn" data-tab="one_time">單次購買</button>
  </div>

  <div id="product-options" class="product-grid">
    <!-- 商品列表將由 JavaScript 動態生成 -->
  </div>
</section>

<!-- 步驟 3：確認付款 -->
<section id="payment-section" class="step-section disabled">
  <h2 class="step-title">
    <span class="step-number">3</span>
    確認付款
  </h2>

  <div class="payment-summary">
    <div class="summary-row">
      <span>總金額</span>
      <span id="total-amount" class="amount">$0</span>
    </div>
  </div>

  <button id="pay-btn" class="btn btn-primary btn-large" disabled>
    前往付款
  </button>

  <div id="error-message" class="error"></div>
</section>
```

---


## ⚠️ 重要注意事項

### 1. 技術術語處理

**保留英文的術語**：
- API、SDK、Token、CSRF、OAuth、JWT
- Webhook、Callback、Promise、Async
- HTTP、URL、JSON、UUID

**可中文化的術語**：
- User → 使用者
- Order → 訂單
- Product → 商品
- Payment → 支付
- Subscription → 訂閱
- Error → 錯誤

### 2. 變數命名策略

**混合團隊**：使用英文識別符 + 完整中文註釋

---

## 📦 最終檔案結構

```
金流串接/
├── README.md                   # ⭐ 唯一的主要文檔
├── 重構規劃.md                 # ⭐ 本文件
├── .env.example                # 環境變數範本
├── package.json
├── index.js                    # 應用進入點
├── src/
│   ├── config/
│   │   └── constants.js        # ⭐ 中文化
│   ├── data/
│   │   └── products.js
│   ├── middleware/
│   │   ├── errorHandler.js
│   │   └── security.js
│   ├── routes/
│   │   ├── auth.js
│   │   ├── orders.js
│   │   ├── payment.js          # ⭐ 中文化
│   │   └── subscriptions.js
│   ├── services/
│   │   ├── index.js
│   │   ├── webhookHandler.js   # ⭐ 新增（中文化）
│   │   ├── database/
│   │   │   ├── provider.js
│   │   │   └── GoogleSheets.js
│   │   └── payment/
│   │       ├── provider.js
│   │       └── PayuniSDK.js
│   └── utils/
│       ├── crypto.js
│       ├── errors.js           # ⭐ 中文化
│       ├── logger.js
│       ├── paymentHelpers.js   # ⭐ 中文化
│       ├── startup.js
│       ├── turnstile.js
│       └── validators.js
├── _frontend/
│   ├── index.html              # ⭐ 中文化
│   ├── orders.html
│   ├── subscriptions.html
│   ├── result.html
│   └── public/
│       ├── js/
│       │   ├── main.js         # ⭐ 新增（中文化）
│       │   └── modules/        # ⭐ 新增（中文化）
│       │       ├── auth.js
│       │       ├── products.js
│       │       ├── payment.js
│       │       ├── orders.js
│       │       └── ui.js
│       ├── style.css
│       ├── landing.css
│       ├── orders.js           # ⭐ 可模組化
│       ├── subscriptions.js    # ⭐ 可模組化
│       ├── result.js
│       └── images/
└── tests/                      # ⭐ 新增（選做）
    ├── utils/
    └── services/
```

---

## 🎯 執行順序與時間規劃

### Week 1：文檔與基礎中文化
**時間**：每天 2-4 小時

1. ✅ **Day 1-2**：Task 1.1 + 1.2 - 文檔整理
   - 建立 README.md
   - 刪除冗餘文檔
   - 測試文件是否正確

2. ✅ **Day 3-4**：Task 5.1 + 5.2 - 後端基礎中文化
   - 改造 `constants.js`
   - 改造 `errors.js`
   - 更新所有 import 路徑

3. ✅ **Day 5-7**：Task 5.3 - 日誌訊息中文化
   - 搜尋所有 `logger` 呼叫
   - 逐一改為中文訊息
   - 測試日誌輸出

### Week 2：前端模組化與中文化
**時間**：每天 2-4 小時

4. ✅ **Day 8-10**：Task 3.1 (Steps 1-3) - 建立前端模組結構
   - 建立 `modules/` 資料夾
   - 建立 `auth.js`（中文化）
   - 建立 `products.js`（中文化）

5. ✅ **Day 11-12**：Task 3.1 (Steps 4-5) - 完成前端模組
   - 建立 `payment.js`（中文化）
   - 建立 `ui.js`（中文化）

6. ✅ **Day 13-14**：Task 3.1 (Steps 6-7) + Task 5.5 - 整合與 HTML
   - 建立 `main.js`（中文化）
   - 更新 `index.html`（中文化）
   - 測試前端功能

### Week 3：後端重構與品質提升
**時間**：每天 2-4 小時

7. ✅ **Day 15-17**：Task 2.3 - Webhook 邏輯提取
   - 建立 `webhookHandler.js`（中文化）
   - 重構 `payment.js` 路由
   - 測試 Webhook 功能

8. ✅ **Day 18-19**：Task 2.2 + 2.4 - SDK 優化
   - 優化 `PayuniSDK.js`
   - 提取共用錯誤處理
   - 新增錯誤類型

9. ✅ **Day 20-21**：Task 4.1 + 4.2 - 品質提升
   - 新增 JSDoc（中文）
   - 統一命名規範
   - Code Review

### Week 4：測試與文檔完善（選做）
**時間**：每天 1-2 小時

10. ✅ **Day 22-24**：Task 4.3 - 單元測試
    - 安裝測試框架
    - 撰寫關鍵函數測試
    - 執行測試確保功能正常

11. ✅ **Day 25-28**：最終檢查
    - 完整功能測試
    - 中文化檢查
    - 更新 README.md
    - 建立部署文檔

---

## ✅ 驗收標準

### 文檔
- [ ] 只保留 `README.md`、`.env.example`、`重構規劃.md`
- [ ] README 包含完整的設定、API、部署說明
- [ ] 所有冗餘文檔已刪除
- [ ] README 完全使用繁體中文

### 前端
- [ ] `app.js` 拆分為 5 個模組
- [ ] 使用 ES6 模組（`import/export`）
- [ ] 所有模組完全中文化
- [ ] 無全域變數污染
- [ ] HTML 所有文字使用繁體中文
- [ ] 功能測試通過

### 後端
- [ ] Webhook 邏輯提取為獨立 service
- [ ] 所有公開方法有中文 JSDoc
- [ ] 錯誤處理統一使用中文化的 `錯誤類型`
- [ ] 回應格式一致
- [ ] 所有 logger 訊息使用中文
- [ ] 常數與配置中文化

### 程式碼品質
- [ ] 命名規範統一（中文或英文+中文註釋）
- [ ] 無重複邏輯
- [ ] Logger 正確使用
- [ ] 程式碼可讀性提升
- [ ] 所有註釋使用繁體中文

### 中文化
- [ ] 使用者可見文字 100% 繁體中文
- [ ] 錯誤訊息 100% 繁體中文
- [ ] 日誌訊息 100% 繁體中文
- [ ] 註釋 100% 繁體中文
- [ ] 技術術語保持一致性

---

## 🚀 執行重構的最佳實踐

### 1. 每次只做一個 Task
避免大規模改動導致難以除錯

### 2. 測試先行
每完成一個 Task 就測試相關功能，確保沒有破壞既有功能

### 3. 使用 Git 版本控制
```bash
# 每個 Task 完成後 commit
git add .
git commit -m "feat: 完成 Task 1.1 - 建立 README.md"

# 建議使用 branch
git checkout -b refactor/frontend-modules
```

### 4. 保留舊檔案（過渡期）
```bash
# 重構前先備份
cp app.js app.js.backup

# 或使用 Git branch
git checkout -b backup/before-refactor
```

### 5. 漸進式重構
不要一次改動太多檔案，建議順序：
1. 先改文檔
2. 再改後端常數與錯誤
3. 然後改前端模組
4. 最後品質提升

### 6. 測試清單
每個階段完成後，執行以下測試：
- [ ] 使用者登入登出
- [ ] 商品列表顯示
- [ ] 一次性支付流程
- [ ] 訂閱制支付流程
- [ ] Webhook 通知處理
- [ ] 訂單歷史查詢

---

## 📚 參考資源

- **ES6 模組**：https://developer.mozilla.org/en-US/docs/Web/JavaScript/Guide/Modules
- **JSDoc**：https://jsdoc.app/
- **Express 最佳實踐**：https://expressjs.com/en/advanced/best-practice-performance.html
- **JavaScript 繁體中文命名規範**：支援 Unicode 識別符，但建議團隊統一標準

---

## 💡 常見問題 FAQ

### Q1: 中文變數名稱會有兼容性問題嗎？
**A**: 現代 JavaScript (ES6+) 完全支援 Unicode 識別符，包括中文。Node.js、瀏覽器都沒有問題。

### Q2: 完全中文化會影響國際化嗎？
**A**: 本專案為內部系統，完全中文化可提升團隊可讀性。若需國際化，建議使用 i18n 方案。

### Q3: 如何處理第三方套件的英文命名？
**A**: 保持第三方套件原有命名，只在業務邏輯層面中文化。例如：
```javascript
import express from 'express'; // 保持原樣
const 應用程式 = express();    // 業務層中文化
```

### Q4: Git commit message 要用中文嗎？
**A**: 建議使用英文 convention（feat、fix、docs 等）+ 中文描述：
```bash
git commit -m "feat: 新增支付管理器模組"
git commit -m "fix: 修正訂單重複建立問題"
```

### Q5: 遇到問題怎麼辦？
**A**:
1. 先回退到上一個 commit
2. 檢查 console 錯誤訊息
3. 使用 `console.log` 除錯
4. 參考本文件的範例程式碼
5. 向團隊 Senior 工程師請教

---

## 🎉 結語

這份重構規劃涵蓋了：
1. **文檔精簡** - 只保留必要文件
2. **程式碼模組化** - 提升可維護性
3. **繁體中文化** - 提升可讀性
4. **品質提升** - 統一規範、新增註釋

預估完成時間：**3-4 週**（每天 2-4 小時）

請按照順序執行，每完成一個 Task 就測試，確保功能正常。

**重構完成後，你將擁有一個結構清晰、易於維護、完全中文化的金流系統！**

---

**最後更新**：2025-12-01
**維護者**：開發團隊
**版本**：v1.0
