...
export const PaymentErrors = {
   ...
   * 使用者未登入或認證失敗
   */
  Unauthorized: (message: string = '請先登入後再操作'): PaymentError =>
   ...
   * 請求參數不正確或格式有誤
   */
  BadRequest: (message: string = '請求參數錯誤', context: Record<string, any> = {}): PaymentError =>
   ...
   * Turnstile 機器人驗證失敗
   */
  TurnstileVerificationFailed: (): PaymentError =>
   ...
   */
  InvalidWebhookSignature: (): PaymentError =>
   ...
   * 查詢的資源無法找到
   */
  NotFound: (message: string = '找不到指定資源', context: Record<string, any> = {}): PaymentError =>
   ...
   * 指定的商品編號不存在於系統中
   */
  ProductNotFound: (productId: string): PaymentError =>
   ...
   * 訂單已完成支付，無法重複付款
   */
  OrderAlreadyPaid: (tradeNo: string): PaymentError =>
   ...
   * 訂閱商品缺少必要的配置資訊
   */
  InvalidSubscriptionConfig: (productId: string): PaymentError =>
   ...
   * 支付金額與訂單金額不符
   */
  AmountMismatch: (expected: number, actual: number): PaymentError =>
   ...
   * 無法連接到資料庫
   */
  DatabaseConnectionFailed: (context: Record<string, any> = {}): PaymentError =>
   ...
   * 資料庫查詢操作失敗
   */
  DatabaseQueryFailed: (context: Record<string, any> = {}): PaymentError =>
   ...
   * 資料庫更新操作失敗
   */
  DatabaseUpdateFailed: (context: Record<string, any> = {}): PaymentError =>
   ...
   */
  OrderQueryFailed: (tradeNo: string): PaymentError =>
   ...
   * 支付提供商 API 響應超時
   */
  APITimeout: (context: Record<string, any> = {}): PaymentError =>
   ...
   * 支付提供商或相關服務不可用
   */
  ServiceUnavailable: (context: Record<string, any> = {}): PaymentError =>
   ...
   * 伺服器處理請求時發生未預期的錯誤
   */
  ServerError: (message: string = '伺服器處理失敗，請稍後重試', context: Record<string, any> = {}): PaymentError =>
