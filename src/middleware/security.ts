    ...
    helmet({
      contentSecurityPolicy: {
        directives: {
          ...
          connectSrc: ["'self'", "https://challenges.cloudflare.com", process.env.DOMAIN].filter((src): src is string => src !== undefined),
 ...
 * 建立通用速率限制器
 */
export function createGeneralLimiter(): any {
  return rateLimit({
 ...
 * 建立支付速率限制器
 */
export function createPaymentLimiter(): any {
  return rateLimit({
 ...
 * 建立 API 結果速率限制器
 */
export function createApiResultLimiter(): any {
  return rateLimit({
 ...
 * 配置 CSRF 防護
 */
export function configureCsrfProtection(app: Express): any {
 ...
 * 配置請求日誌中間件
 */
export function configureRequestLogger(app: Express): void {
  app.use((req: Request, res: Response, next: NextFunction) => {
