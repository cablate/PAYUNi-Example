...
你決定自己串金流時，會碰上什麼挑戰？帶著這些真實的問題，我們一起探索背後的設計原則——這些原則不只適用於金流，更是任何複雜系統都該遵循的核心思維。
...
**這些問題引出的原則：關注點分離（Separation of Concerns）**
...
這份教學包最核心的目標不是「教你怎麼串 Payuni API」，而是讓你學會「面對複雜系統時應該怎麼思考」。金流只是一個最好的練習場。
...
當你讀 `01_SYSTEM_ARCHITECTURE.md` 時，心裡帶著這些問題：
...
> 這份教材的定位已依據 `AI_OUTPUTS/Pivoted_Payment_Gateway_Strategy.md` 調整。我們不是在賣「金流串接工具」，而是在教「如何設計複雜系統的思維」。金流只是載體，讓你在真實場景中練習這些原則。

**下一步：** 當你準備好了，前往 `01_SYSTEM_ARCHITECTURE.md`，看看這些原則在真實流程中如何運作。
