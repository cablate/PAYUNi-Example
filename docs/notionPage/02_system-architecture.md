...
| 元件 | 它負責什麼 | 它不負責什麼 | 💀 如果你不這樣做會怎樣 (Risk) |
|------|----------|-----------|------------|
| **前端** | 呈現、收集使用者輸入、呼叫 API | 驗證、加密、儲存敏感資料 | **使用者可以隨便改前端 Code**。如果你在前端算錢，駭客會直接用 $1 元買走你的 $1000 元商品。 |
| **後端** | 驗證、加密、業務邏輯 | 呈現介面、直接處理支付 | **這是你的最後防線**。如果後端沒驗證，所有從前端來的髒資料都會直接汙染資料庫。 |
| **Payuni** | 支付流程、金鑰管理 | 驗證訂單、修改金額 | **金流方只管收錢**。他們不會管你的庫存夠不夠，也不會管這筆訂單是不是重複刷的。 |
| **資料庫** | 儲存最終結果 | 驗證訂單、加密敏感資料 | **資料庫只是倉庫**。不要期待倉庫管理員幫你抓小偷，那是保全（後端）的事。 |

**核心啟示：** 不要相信任何從客戶端（前端）傳來的資料。永遠假設前端已經被駭客控制了。
...
讀到這裡，你已經理解了「使用者登入」和「建立訂單」這兩個相對直線性的流程。接下來的「Webhook 流程」是大多數金流災難的發生地。

> **⚠️ 真實慘案：那個消失的 iPhone 訂單**
> 某個電商網站只靠前端 Return URL 來判斷付款成功。結果駭客發現只要直接訪問 `result.html?status=success`，就能讓網站以為他付錢了。
> 
> 於是，駭客用腳本刷了 100 支 iPhone，後台全顯示「已付款」，倉庫自動出貨。直到月底對帳才發現：**根本沒收到錢**。
>
> **這就是為什麼我們需要 Webhook，而且需要雙重驗證。**
