...
> 先定位 → 再理解原則 → 再看架構 → 補強安全 → 最後動手

| Stage | 目標 | 這一階段讀什麼 | 讀完你會得到 | 下一步 |
|-------|------|----------------|---------------|--------|
| 0. Orientation | 搞懂整套教材的價值與適合族群 | 本頁 `index.md` | 明確的學習路線與期望成果 | → Stage 1 |
| 1. Understand Why | 釐清金流為例的四大挑戰與設計原則 | `01_design-philosophy.md` | 能用共同語言討論「為什麼要這樣設計」 | → Stage 2 |
| 2. Map the System | 把抽象原則映射到具體流程、責任邊界 | `02_system-architecture.md` | 以圖像與分層思維看懂整個系統 | → Stage 3 |
| 3. Secure the Flow | 了解 5 層防線與常見誤區 | `03_security-deep-dive.md` | 知道每條防線保護什麼、怎麼驗收 | → Stage 4 |
| 4. Build & Extend | 把習得的原則用在實際修改、擴充 | `04_customization-guide.md` | 能自信地動手改、寫、驗證 | （可回頭複習或部署） |

💡 建議照順序閱讀：每一階的「輸出」會成為下一階的「輸入」。如果你已熟悉其中一階段，可直接跳到你需要的 Stage，再回來補缺的部分。
